    ...
    <div className="w-72 flex-shrink-0 border-l border-[#1a1a1a] p-6 hidden lg:block">
      ...
      <div className="border border-[#1e1e1e] bg-[#111] rounded overflow-hidden p-4">
        <div className="text-center mb-4">
          ...
          <div className="font-bold text-sm" style={{ fontFamily: "var(--font-barlow)" }}>{profile.display_name || profile.username}</div>
          {profile.bio && <div className="text-[#444] text-xs mt-0.5">{profile.bio}</div>}
        </div>
        <div className="space-y-2">
          ...
          {links.filter((l) => l.is_active).length === 0 && (
            <div className="text-[#2a2a2a] text-xs text-center py-2">링크 없음</div>
          )}
        </div>
      </div>
    </div>
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          {safeProfile.username && (
            <div className="text-right">
              <div className="text-[10px] font-bold tracking-[0.2em] text-[#e10600]/60 uppercase"
                style={{ fontFamily: "var(--font-barlow)" }}>
                내 링크 주소
              </div>
              <div className="text-xs text-[#444] mt-0.5">
                mylink.kr/<span className="text-white">{safeProfile.username}</span>
              </div>
            </div>
          )}
        </div>
